  ...
  methods: {
    ...
    async fetchProducts() {
      ...
      try {
        ...
        // Fallback to local sample data so UI still shows cards
        console.warn('HomeView fetch failed, using sample data:', e.message || e);
        this.products = sampleProducts.map(p => ({ ...p, id: p._id || p.id }));
        this.error = null;
      } finally {
        this.loading = false;
      }
