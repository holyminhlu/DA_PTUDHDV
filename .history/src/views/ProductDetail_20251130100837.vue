    ...
    <main class="container detail-container">
      ...
      <div v-else-if="product">
        <section class="product-summary">
          <h1 class="summary-title">{{ product.title }}</h1>
          <div class="summary-meta">
            <div class="summary-category">{{ product.category || 'Thiết bị' }}</div>
            <div class="summary-price-row">
              <div class="summary-price">{{ formatPrice(product.price) }}</div>
              <div v-if="product.oldPrice" class="summary-old-price">{{ formatPrice(product.oldPrice) }}</div>
            </div>
            <div class="summary-rating">
              <div class="stars" aria-hidden="true">
                <span v-for="(s,i) in ratingArray" :key="i" :class="{ filled: s }">★</span>
              </div>
              <div class="summary-rating-number">{{ product.rating || '0.0' }}</div>
              <div class="summary-reviews">({{ product.reviews || 0 }} đánh giá)</div>
            </div>
          </div>
        </section>
...
.warranty h3, .colors h3, .specs h3 { margin-top:16px; margin-bottom:8px; }
.specs ul { list-style:none; padding:0; margin:0; }
.specs li { padding:6px 0; border-bottom:1px dashed #eef2f6; }
.rating { display:flex; align-items:center; gap:12px; margin-bottom:12px; }
.stars { display:flex; align-items:center; gap:6px; }
.stars span { font-size:20px; color:#e6e7ea; }
.stars span.filled { color:#f59e0b; }
.rating-number { margin-left:6px; color:#475569; font-weight:700; }
