  ...
  methods: {
    ...
    async fetchProduct() {
      ...
      try {
        ...
        // First try productinfo (detailed collection PhoneInfo)
        try {
          const { data } = await axios.get(`${base}/api/productinfo/${id}`);
...
<style scoped>
.detail-container { max-width: 1200px; margin: 32px auto; padding: 0 24px; }
