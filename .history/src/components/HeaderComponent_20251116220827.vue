...
export default { 
  ...
  methods: {
    ...
    isActive(item) {
      // prefer named routes when provided
      if (item && item.to && item.to.name) {
        return this.$route && this.$route.name === item.to.name;
      }
      if (item && item.path) {
        return this.$route && this.$route.path === item.path;
      }
      return false;
    },
    formatPrice(price) {
