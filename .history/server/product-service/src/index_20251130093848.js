...
app.get('/phoneinfo/:id', async (req, res) => {
    ...
    try {
        ...
        // 1. Try matching the 'id' field (string or numeric)
        ...
        // 2. If still not found, try matching by ObjectId (_id)
        if (!phone && mongooseId.isValid(id)) {
            phone = await PhoneInfo.findById(id).lean();
        }

        // 3. As a last step, try phoneId field (string or number)
        if (!phone) {
            phone = await PhoneInfo.findOne({ phoneId: id }).lean() || await PhoneInfo.findOne({ phoneId: Number(id) }).lean();
        }
